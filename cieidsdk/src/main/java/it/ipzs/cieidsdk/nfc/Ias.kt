 ...
 */
open class Ias constructor(private val isoDep: IsoDep) {

    internal class InternalAuthenticationException(message: String) :
    ...
    fun getIdServizi(): String {
        CieIDSdkLogger.log("getIdServizi()", false)
    ...
    private fun verifyPin(pin: String): Int {
        CieIDSdkLogger.log("verifyPin()", false)
        if (pin.length != 8) {
    ...
    private fun dApp() {
        ...
        val cert = AppUtil.asn1Tag(
            AppUtil.appendByteArray(AppUtil.appendByteArray(tmp, tmp1), tmp2),
            0x7f21
        )
        ...
        val PRND: ByteArray = AppUtil.getRandomByte(padSize)
        ...
        val rndIFD: ByteArray = AppUtil.getRandomByte(8)
        ...
        intAuthResp =
            rsaIntAuthKey.encrypt(AppUtil.getSub(resp.response, 8, resp.response.size - 8))
    ...
    fun readCertCie(): ByteArray {
        CieIDSdkLogger.log("readCie()", false)
        //selectAidCie()
    ...
    private fun dhKeyExchange() {
        CieIDSdkLogger.log("dhKeyExchange()", false)
    ...
    private fun initExtAuthKeyParam() {
        CieIDSdkLogger.log("initExtAuthParam()", false)
        //selectAidCie()
    ...
    private fun initDHParam() {
        CieIDSdkLogger.log("initDHParam()", false)
        //selectAidCie()
    ...
    private fun readDappPubKey() {
        CieIDSdkLogger.log("readDappPubKey()", false)
    ...
    private fun selectAidCie() {
        CieIDSdkLogger.log("selectAidCie()", false)
    ...
    private fun selectAidIas() {
        CieIDSdkLogger.log("selectAidIas()", false)
    ...
    private fun readFileSM(id: Int): ByteArray {
        ...
        while (true) {
            ...
            if (response.swShort == ApduResponse.SwError.SW_NO_ERROR) {
                content = AppUtil.appendByteArray(content, chn)
                cnt += chn.size
                chunk = 256
            } else {
                if (response.swShort == ApduResponse.SwError.SW_END_OF_FILE) {
                    content = AppUtil.appendByteArray(content, chn)
                } else if (response.swShort != ApduResponse.SwError.SW_WRONG_P1P2) {
                    return content
                }
                break
            }
    ...
    private fun sendApduSM(head: ByteArray, data: ByteArray, le: ByteArray?): ApduResponse {
        ...
        if (ds < unsignedToBytes(0xe7.toByte())) {
            ...
            val cla = head[0]
            while (true) {
                val s = AppUtil.getSub(data, i, (data.size - i).coerceAtMost(0xE7))
                i += s.size
                if (i != data.size)
                    head[0] = (cla or 0x10)
                else
    ...
    private fun sendApdu(head: ByteArray, data: ByteArray, le: ByteArray?): ApduResponse {
        ...
        if (ds > 255) {
            ...
            while (true) {
                apdu = byteArrayOf()
                val s = AppUtil.getSub(data, i, (data.size - i).coerceAtMost(255))
                i += s.size
                if (i != data.size)
                    head[0] = (cla or 0x10)
                else
    ...
    @Throws(Exception::class)
    private fun respSM(keyEnc: ByteArray, keySig: ByteArray, respParam: ByteArray): ApduResponse {
        AppUtil.increment(seq)
        // cerco il tag 87
        setIndex(0)
        ...
        do {
            ...
            if (resp[index].compareTo(0x8e.toByte()) == 0) {
                val calcMac = Algoritmi.macEnc(
                    ...
                    AppUtil.getIsoPad(
                        AppUtil.appendByteArray(
                            AppUtil.appendByteArray(seq, encObj),
                            dataObj
                        )
                    )
                )
            ...
            if (resp[index] == 0x87.toByte()) {
                ...
                if (resp[index + 1] > 0x80.toByte()) {
                    ...
                    encObj = AppUtil.getSub(resp, index, llen + lgn + 2)
                    encData =
                        AppUtil.getSub(resp, index + llen + 2, lgn) // ' levo il padding indicator
                    setIndex(index, llen, lgn, 2)//index += llen + lgn + 2;
                } else {
                    encObj = AppUtil.getSub(resp, index, resp[index + 1] + 2)
        ...
        if (encData.isNotEmpty()) {
            resp = Algoritmi.desDec(keyEnc, encData)
    ...
    @Throws(Exception::class)
    private fun getRespSM(responseTmpParam: ApduResponse): ApduResponse {
        var responseTmp = responseTmpParam
    ...
    private fun transmit(apdu: ByteArray): ApduResponse {
        CieIDSdkLogger.log("APDU: " + AppUtil.bytesToHex(apdu), false)
        val resp = ApduResponse(isoDep.transceive(apdu))

        CieIDSdkLogger.log("RESPONSE: " + AppUtil.bytesToHex(resp.response), false)
        CieIDSdkLogger.log("SW: " + AppUtil.bytesToHex(resp.swByte), false)
        return resp
    }
        ...
        Thread {
            try {
                while (rlSign.tryLock()) {
                    try {
                        sendApduSM(
                            byteArrayOf(0x00, 0xa4.toByte(), 0x02, 0x04),
                            byteArrayOf(HIBYTE(id), LOBYTE(id)),
                            null
                        )
                    } finally {
                        rlSign.unlock()
